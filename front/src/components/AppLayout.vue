  ...
  <el-container class="app-layout">
    <el-aside :width="app.sidebarCollapsed ? '64px' : '220px'" class="app-sidebar">
      ...
      <el-menu :default-active="activeMenu" :collapse="app.sidebarCollapsed" router background-color="#304156" text-color="#bfcbd9" active-text-color="#409EFF">
        ...
        <el-sub-menu index="/resources" v-if="auth.hasPermission('campus:read') || auth.hasPermission('building:read') || auth.hasPermission('floor:read') || auth.hasPermission('room:read') || auth.hasPermission('workstation:read') || auth.hasPermission('facility:read') || auth.hasPermission('policy:read')">
        ...
        <el-sub-menu index="/notifications" v-if="auth.hasPermission('webhook:read')">
          ...
          <el-menu-item index="/notifications" v-if="auth.hasPermission('webhook:read')">Webhook 配置</el-menu-item>
          <el-menu-item index="/notifications/audit" v-if="auth.hasPermission('webhook:read')">投递审计</el-menu-item>
        </el-sub-menu>

        <el-sub-menu index="/system" v-if="auth.hasPermission('user:read') || auth.hasPermission('role:read') || auth.hasPermission('department:read')">
